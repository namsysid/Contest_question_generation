#!/usr/bin/env python3
"""
Naive all-in-one RAG over question text only.

Pipeline:
1) Read question corpus JSONL and extract question text.
2) Embed all question texts.
3) For each sampled seed question, retrieve top-k nearest question exemplars (naive cosine).
4) Generate one new multiple-choice question from the retrieved context.
5) Write generated rows to JSONL.
"""
...
SYSTEM_GEN = """You generate novel, contest-style STEM multiple-choice questions from retrieved question-only exemplars.

Hard constraints:
- Do NOT copy exemplar wording, numbers, variable names, or scenarios.
- Produce one self-contained question with exactly 5 choices (A)-(E).
- Include a single correct answer and a concise solution.
...
def build_prompt(seed: Dict[str, str], exemplars: List[Dict[str, str]], max_chars: int) -> str:
    ex_text = "\n\n".join(
...
TASK:
Generate ONE new hard contest-style multiple-choice question with 5 choices (A)-(E), one correct answer, and a concise but complete solution.
Use retrieval only as style/topic prior. Keep the new question novel.
...
def llm_json_with_retries(model: str, prompt: str, retries: int, temperature: float) -> Dict[str, Any]:
    last_exc: Optional[Exception] = None
    for attempt in range(1, max(1, retries) + 1):
        try:
            return generate_json(model, prompt, system=SYSTEM_GEN, temperature=temperature)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="All-in-one naive RAG over question text only.")
    ap.add_argument("--input", default="data/txts/all_questions.jsonl")
    ap.add_argument("--out", default="ablation_1/generated/naive_rag_questions.jsonl")
    ap.add_argument("--bundles-out", default="ablation_1/retrieval_bundles.jsonl", help="Retrieval audit JSONL output.")
    ap.add_argument("--embeddings-out", default="ablation_1/question_embedded.jsonl", help="Question embedding dump JSONL.")
    ap.add_argument("--embed-model", default="qwen3-embedding")
    ap.add_argument("--model", default="qwen2.5:7b-instruct")
    ap.add_argument("--num", type=int, default=25, help="Number of questions to generate.")
    ap.add_argument("--k", type=int, default=6, help="Number of nearest exemplars per seed.")
    ap.add_argument("--limit", type=int, default=0, help="If > 0, only use the first N usable input rows.")
    ...
    for i, bundle in enumerate(bundles, start=1):
        prompt = build_prompt(bundle["seed"], bundle["retrieved_exemplars"], args.max_chars)
        try:
            cand = llm_json_with_retries(args.model, prompt, retries=args.json_retries, temperature=args.temperature)
            if not schema_ok(cand):
                raise ValueError("Generated JSON failed schema checks.")

            out_rows.append(
                {
                    **cand,
                    "id": cand.get("id") or f"naive_rag_{i:05d}",
                    "_meta": {
                        "generator": "all_in_one_naive_rag_questions",
    ...
    print(f"Wrote {len(out_rows)} generated rows -> {args.out} (failures={failures})")
